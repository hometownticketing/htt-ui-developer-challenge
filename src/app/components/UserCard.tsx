...
export const UserCard = ({ className, id = 0, name, title, subTitle, bodyText, children, count = 0, quantity = 0, link = '#', linkText, onClick, dataTest, isActive }: CardInterface) => {
  const cardId = `${ id }`;

  return (
    <Card dataTest="card" id={cardId} isActive={isActive} className={className}>
      {name && (
